# Xboard-Node 隐藏进程多面板安装脚本 (Debian/Ubuntu)
...
一个专为 **Debian/Ubuntu** 系统设计的 Xboard-Node **隐藏进程**多面板安装脚本，支持同一台服务器对接多个面板，且进程名完全隐藏。

> 💡 进程名在 `ps -ef` 中显示为随机生成的系统名称（如 `system-a1b2c3d4`），完全隐藏 xboard 字样。

## 功能特性

- 🔒 **隐藏进程名** - 进程显示为随机系统名称（如 `system-xxxx`），完全隐藏
- ✅ **多面板支持** - 一台服务器对接多个不同面板
- ✅ **独立实例** - 每个实例独立运行，互不影响
...
| 功能 | 普通版 | 隐藏进程版 |
|------|--------|------------|
| 进程名 | `xboard-node` | `system-xxxxxxx` |
| `ps -ef` | 可见 | 不可见 |
| 安装复杂性 | 简单 | 稍复杂 |
| 卸载复杂性 | 简单 | 稍复杂 |
...
1. **随机进程名**：安装时生成随机名称，如 `system-a1b2c3d4`
2. **包装脚本**：创建包装脚本执行真正的 xboard-node
3. **systemd 服务**：服务描述为普通系统服务名称
...
# 普通版 - 可见 xboard
$ ps -ef | grep xboard
root  1234  1  0 10:00 ?  00:00:00 /usr/local/bin/xboard-node -c ...
...
$ ps -ef | grep system
root  1234  1  0 10:00 ?  00:00:00 /usr/local/bin/system-a1b2c3d4 -c ...
```
...
```bash
# 使用管理脚本
sudo /usr/local/bin/service-manager start
sudo /usr/local/bin/service-manager stop
```
...
| 包装脚本 | `/usr/local/bin/system-{随机字符}` |
| systemd 服务 | `/etc/systemd/system/xboard-node-{实例名}.service` |
| 管理脚本 | `/usr/local/bin/service-manager` |

## 常见问题

### Q: 如何查看隐藏的进程？

A: 使用 `ps -ef | grep system-` 可以看到隐藏后的进程名。原始的 `ps -ef | grep xboard` 不会显示任何结果。
